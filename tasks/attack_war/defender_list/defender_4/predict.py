...
class Prediction():
    ...
    def __init__(self, device, file_path):
        self.device = device
        self.model = self.constructor(file_path)[0].to(device)
        self.detector = self.constructor(file_path)[1].to(device)
    ...
    def constructor(self, file_path=None):
        ...
        if file_path != None:
            ...
            detector.load_state_dict(torch.load(file_path+'/attack_detector-model.pth', map_location=self.device))
        model.eval()
        detector.eval()
    ...
    def preprocess(self, original_images):
        ...
        for i in range(K):
          perturbed_image = perturbed_image - step_size * 0.275
    ...
    def detect_attack(self, original_image):
        pred = self.detector(original_image.to(self.device))
